...
module.exports = {
    ...
    getUsedTickets(user_id, callback){
        const baseQuery = 'SELECT ticket.place costumer.name, event.name, event.description, event.place, event.price, event.date'
            + ' FROM ticket, costumer, event WHERE ticket.costumer_id = ? AND ticket.is_used = TRUE AND event.id = ticket.event_id AND costumer.name = ticket.costumer_id';
    ...
    getNotUsedTickets(user_id, callback){
        const baseQuery = 'SELECT ticket.place costumer.name, event.name, event.description, event.place, event.price, event.date'
            + ' FROM ticket, costumer, event WHERE ticket.costumer_id = ? AND ticket.is_used = FALSE AND event.id = ticket.event_id AND costumer.name = ticket.costumer_id';
        execute(baseQuery, [user_id], (response, err) => {
            if(err){
                callback(null, err);
            }
    ...
    getAllTickets(user_id, callback){
        const baseQuery = 'SELECT ticket.is_used, ticket.place costumer.name, event.name, event.description, event.place, event.price, event.date'
            + ' FROM ticket, costumer, event WHERE ticket.costumer_id = ? AND event.id = ticket.event_id AND costumer.name = ticket.costumer_id';
        execute(baseQuery, [user_id], (response, err) => {
            if(err){
                callback(null, err);
            }
